...
import type { CustomError } from "./abstractions/CustomError";
import { MAX_ERROR_LOG_SIZE } from "./consts/LogConsts";
...
export class ConsoleErrorLogger extends Logger {
	...
	public logError(errorMsg: string, stack?: string, originalError?: Error) {
		const error = this.getError(errorMsg, ErrorLevel.Error, stack, originalError);
	...
	public logWarning(warningMsg: string, stack?: string, originalError?: Error) {
		const warning = this.getError(warningMsg, ErrorLevel.Warning, stack, originalError);
	...
	public logMessage(logMsg: string, stack?: string, originalError?: Error) {
		const log = this.getError(logMsg, ErrorLevel.Log, stack, originalError);
	 ...
	 */
	private addMessageToErrorLog(error: CustomError): void {
		// Add the new error
